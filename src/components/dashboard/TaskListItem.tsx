"use client";
import { CheckCircle2, Star, Trash2, Clock, Archive, MoreVertical } from "lucide-react";
import { cn } from "@/lib/utils";
import { Task, Member, Priority, useStore } from "@/lib/store";
  ...
  return (
    <div
      ...
      className={cn(
        ...
        task.isCompleted ? "bg-slate-50/50 dark:bg-white/[0.01] text-slate-500" : "bg-white dark:bg-[#161b22] text-slate-900 dark:text-slate-100 font-medium",
    ...
    >
      ...
      <div className="flex items-center gap-3 shrink-0" onClick={e => e.stopPropagation()}>
        <div
          ...
          className={cn("h-4 w-4 rounded border transition-all flex items-center justify-center",
            task.isCompleted ? "bg-indigo-600 border-indigo-600" : "border-slate-300 dark:border-slate-600 hover:border-slate-400"
          )}
        >
        ...
        <button
          ...
          className={cn("transition-all hover:scale-110", isHigh ? "text-amber-400" : "text-slate-300 dark:text-slate-600 hover:text-slate-400")}
        >
      ...
      <div className="w-32 hidden md:block shrink-0 truncate text-sm">
        <span className={cn("px-2 py-0.5 rounded text-[11px] font-bold mr-2",
        ...
        )}>{task.priority[0]}</span>
        <span className={!task.isCompleted ? "font-bold" : ""}>{assignee?.name || "Unassigned"}</span>
      </div>

      {/* Title & meta */}
      <div className="flex-1 min-w-0 flex items-center gap-2">
        <span className={cn("truncate text-sm", !task.isCompleted ? "font-bold text-slate-900 dark:text-slate-100" : "text-slate-500")}>
          {task.title}
        </span>
        ...
        {project && (
          <div className="flex items-center gap-1.5 ml-2 shrink-0">
            <div className={cn("w-2 h-2 rounded-full", project.color || "bg-indigo-500")} />
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{project.name}</span>
          </div>
        )}
        {task.tags && task.tags.length > 0 && (
          <div className="flex gap-1 ml-2 shrink-0">
            {task.tags.slice(0, 1).map(tag => (
              <span key={tag} className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/5 text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{tag}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-4 shrink-0 min-w-[80px] justify-end">
        ...
        <span className={cn("text-xs group-hover:hidden", !task.isCompleted ? "font-bold text-slate-900 dark:text-slate-300" : "text-slate-400")}>
